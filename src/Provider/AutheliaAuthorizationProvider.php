...
use Jefferson49\OAuth2\Client\Provider\Authelia;
use Jefferson49\OAuth2\Client\Provider\AutheliaResourceOwner;
...
/**
 * An OAuth2 authorization client for Authelia
 */
class AutheliaAuthorizationProvider extends AbstractAuthorizationProvider implements AuthorizationProviderInterface
{        
    ...
    public function __construct(string $redirectUri, array $options = [], array $collaborators = [])
    {
        ...
        $this->provider = new Authelia($options, $collaborators);
    }
    ...
    public function getUserData(AccessToken $token) : AuthorizationProviderUser {
        ...
        //Apply specific user data provided by Authelia
        /** @var AutheliaResourceOwner $resource_owner To avoid IDE warnings */
        $user->setUserName($resource_owner->getPreferredUsername() ?? '');
        $user->setRealName($resource_owner->getPreferredUsername() ?? '');
